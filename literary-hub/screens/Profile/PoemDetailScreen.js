...
const Poem = ({ route }) => {
  ...
  console.log("hello entered poem detail screen");
  console.log(poem);
  ...
  const fetchPoem = async () => {
    try {
      ...
      //console.log("this is the current poem the current poem the current poem");
  ...
  const getPoemPages = async () => {
    try {
      poemToPage(poem, linesPerPage);
      poemToPage(currentPoem, linesPerPage);
      console.log("they have been paged");
    } catch (error) {
      console.log("error", error);
    }
  };
  ...
  return (
    <View>
      ...
      >
        ...
        >
          {currentPoem.pages.map((page, index) => (
            <View
              key={index}
              ...
              {index === 0 && (
                <React.Fragment>
                  <View style={styles.titleBox}>
                    <Text style={styles.title}>{currentPoem.title}</Text>
                  ...
                  <Text style={styles.author}>by {currentPoem.author}</Text>
                </React.Fragment>
              )}
        ...
        <View style={styles.pagination}>
          <Dots
            length={currentPoem?.pages?.length || 10}
            active={activePage}
            activeColor="#644980"
            passiveColor="#C3CBCD"
          />
